...
void AEnemyShipTransport::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AEnemyShipTransport::Tick(float DeltaTime)
{
...
AEnemyShipTransport::AEnemyShipTransport()
{
	// Crear el mesh del enemigo
	static ConstructorHelpers::FObjectFinder<UStaticMesh> EnemyMeshAsset(TEXT("StaticMesh'/Game/StarterContent/Shapes/Shape_Tube.Shape_Tube'"));
	EnemyMesh->SetStaticMesh(EnemyMeshAsset.Object);

}

// Movimiento de la nave hunter (sigue al jugador)
void AEnemyShipTransport::MoveEnemy(float DeltaTime)
{
